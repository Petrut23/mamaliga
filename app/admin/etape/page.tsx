...
export default function AdminEtapePage() {
  ...
  const [form, setForm] = useState({ seasonId: "", roundNumber: "", title: "", deadlineDate: "", deadlineTime: "23:00" })
  ...
  function startEdit(etapa: any) {
    ...
    const localDate = new Date(deadline.getTime() + 3 * 60 * 60 * 1000)
    const dateStr = localDate.toISOString().slice(0, 10)
    const timeStr = localDate.toISOString().slice(11, 16)
    setForm({ seasonId: etapa.seasonId, roundNumber: String(etapa.roundNumber), title: etapa.title, deadlineDate: dateStr, deadlineTime: timeStr })
    setShowForm(true)
  }

  function cancelEdit() {
    setEditingEtapa(null)
    setForm({ seasonId: "", roundNumber: "", title: "", deadlineDate: "", deadlineTime: "23:00" })
    setShowForm(false)
  }
  ...
  async function saveEtapa(e: any) {
    ...
    if (editingEtapa) {
      const res = await fetch("/api/admin/etape", {
        ...
        body: JSON.stringify({ id: editingEtapa.id, ...form, roundNumber: parseInt(form.roundNumber), deadlineAt: deadlineUTC })
      })
      if (res.ok) { setMsg("Etapa actualizata!"); cancelEdit(); loadEtape() }
    } else {
      const res = await fetch("/api/admin/etape", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, roundNumber: parseInt(form.roundNumber), deadlineAt: deadlineUTC })
      })
      const data = await res.json()
      if (res.ok) { setMsg("Etapa creata!"); setShowForm(false); loadEtape() }
    }
  }
  ...
  async function deleteEtapa(id: string) {
    ...
    if (res.ok) { setEtape((prev: any) => prev.filter((e: any) => e.id !== id)); setMsg("Etapa stearsa!") }
  }

  async function updateStatus(id: string, status: string) {
    await fetch("/api/admin/etape", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status }) })
    ...
    <div className="min-h-screen bg-[#0a0d14] text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-3xl font-black mb-1">Etape</h1><p className="text-gray-500 text-sm">Gestioneaza etapele sezonului</p></div>
          <button onClick={() => { cancelEdit(); setShowForm(!showForm) }} className="bg-[#e8ff47] text-black font-bold px-5 py-2.5 rounded-lg hover:bg-[#f5ff6e] transition-colors">+ Etapa noua</button>
        </div>
        ...
        {showForm && (
          <div className="bg-[#111520] border border-[#1e2640] rounded-xl p-6 mb-8">
            ...
            <form onSubmit={saveEtapa} className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm text-gray-400 mb-1">Sezon</label><select value={form.seasonId} onChange={e => setForm({...form, seasonId: e.target.value})} className="w-full bg-[#0a0d14] border border-[#1e2640] rounded-lg px-3 py-2 text-white" required><option value="">Selecteaza sezon</option>{sezoane.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
              <div><label className="block text-sm text-gray-400 mb-1">Numar etapa</label><input type="number" value={form.roundNumber} onChange={e => setForm({...form, roundNumber: e.target.value})} className="w-full bg-[#0a0d14] border border-[#1e2640] rounded-lg px-3 py-2 text-white" required /></div>
              <div className="col-span-2"><label className="block text-sm text-gray-400 mb-1">Titlu</label><input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full bg-[#0a0d14] border border-[#1e2640] rounded-lg px-3 py-2 text-white" required /></div>
        ...
        {loading ? <div className="text-gray-500 text-center py-20">Se incarca...</div> : etape.length === 0 ? <div className="text-center py-20 text-gray-500">Nu exista etape!</div> : (
            ...
            {etape.map((etapa: any) => (
              <div key={etapa.id} className="bg-[#111520] border border-[#1e2640] rounded-xl px-6 py-4 flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <div className="text-sm text-gray-500 mt-0.5">Deadline: {new Date(etapa.deadlineAt).toLocaleString("ro-RO")}</div>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button onClick={() => startEdit(etapa)} className="text-xs bg-[#3b82f6]/10 text-[#3b82f6] border border-[#3b82f6]/20 px-3 py-1 rounded-lg hover:bg-[#3b82f6]/20">Edit</button>
                  ...
                  <button onClick={() => deleteEtapa(etapa.id)} className="text-xs bg-red-500/10 text-red-400 border border-red-500/20 px-3 py-1 rounded-lg hover:bg-red-500/20">Sterge</button>
                </div>
              </div>
            ))}
          </div>
