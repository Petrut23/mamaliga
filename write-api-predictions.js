...
fs.mkdirSync('app/api/predictions/save', { recursive: true })
fs.mkdirSync('app/api/predictions/my', { recursive: true })

const saveRoute = `import { NextRequest, NextResponse } from "next/server"
...
const myRoute = `import { NextRequest, NextResponse } from "next/server"
import { PrismaClient } from "@prisma/client"
import { getServerSession } from "next-auth"
import { authOptions } from "@/pages/api/auth/[...nextauth]"
export const dynamic = "force-dynamic"
...
fs.writeFileSync('app/api/predictions/save/route.ts', saveRoute)
fs.writeFileSync('app/api/predictions/my/route.ts', myRoute)
console.log('Gata!')