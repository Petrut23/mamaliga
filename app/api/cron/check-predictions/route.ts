...
export async function GET() {
  try {
    ...
    if (webhookUrl) {
      ...
      const message = {
        embeds: [{
          title: `${titlePrefix} Atenție! Nu și-au pus încă predicțiile`,
          description: `Pentru **${round.title}**:\n\n${missingText}`,
          color: 0xff9500,
          fields: [
            { name: "⏰ Deadline", value: deadline, inline: false },
            { name: "🔗 Link", value: `[Pune-ți predicțiile acum](${predictiiLink})`, inline: false }
          ]
        }]
      }
    ...
    if (gmailUser && gmailPass) {
      ...
      for (const user of missingUsers) {
        ...
        await transporter.sendMail({
          ...
          to: user.email,
          subject: `${titlePrefix} Nu ai pus încă predicțiile pentru ${round.title}!`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0d14; color: white; padding: 40px; border-radius: 12px;">
              ...
              <h2 style="color: white; font-size: 22px; margin-bottom: 16px;">${titlePrefix} Nu ai pus încă predicțiile!</h2>
