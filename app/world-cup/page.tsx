...
export default function WCPredictiiPage() {
  ...
  async function savePredictions() {
    ...
    if (missingScores.length > 0) { showToast("⚠️ Ai " + missingScores.length + " meciuri fara scor!", "error"); return }
    if (!captain) { showToast("⚠️ Trebuie sa alegi un meci capitan!", "error"); return }
    setSaving(true)
  ...
  if (!round) return (
    <div className="min-h-screen bg-[#0a0d14] flex items-center justify-center">
      <div className="text-center">
        ...
        <div className="text-gray-500">Adminul nu a creat inca o etapa pentru World Cup.</div>
              ...
              <div className={"pointer-events-auto max-w-sm w-full mx-auto rounded-2xl px-6 py-5 shadow-2xl transition-all duration-300 " + (toastVisible ? "opacity-100 scale-100" : "opacity-0 scale-95") + " " + (toastType === "error" ? "bg-[#1a0a0a] border border-red-500/40" : "bg-[#0a1a0a] border border-green-500/40")} style={{backdropFilter: "blur(20px)"}}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs md:text-sm text-gray-300 mt-0.5">{toast.replace("✅ ", "").replace("⚠️ ", "").replace("❌ ", "")}</div>
                  </div>
                </div>
              </div>
            </div>
