...
export async function GET() {
  try {
    ...
    // Etapa anterioara (COMPLETED) - diferita de cea activa
    const previousRound = await prisma.round.findFirst({
      where: { 
        status: "COMPLETED",
        id: activeRound ? { not: activeRound.id } : undefined
      },
      orderBy: { createdAt: "desc" }
    })
    ...
    // Daca avem etapa activa LOCKED/LIVE - scenariul 2
    ...
    // Scenariul 1 sau 3 - etapa completata + eventual countdown
    const prevData = previousRound || activeRound
    let prevMatches: any[] = []
    ...
    if (prevData) {
      prevMatches = await prisma.match.findMany({
        where: { roundId: prevData.id },
        orderBy: { kickoffAt: "asc" }
      })
      const prevPredictions = await prisma.prediction.findMany({
        where: { match: { roundId: prevData.id } },
        include: { user: { select: { id: true, name: true } } }
      })
