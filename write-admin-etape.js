...
fs.mkdirSync('app/admin/etape', { recursive: true })
fs.mkdirSync('app/api/admin/etape', { recursive: true })
fs.mkdirSync('app/api/admin/sezoane', { recursive: true })

// Pagina etape admin
const etapePage = `"use client"
...
export default function AdminEtapePage() {
  ...
  async function createEtapa(e: any) {
    e.preventDefault()
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {showForm && (
          <div className="bg-[#111520] border border-[#1e2640] rounded-xl p-6 mb-8">
            <h2 className="text-lg font-bold mb-4">Creeaza etapa noua</h2>
            <form onSubmit={createEtapa} className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="ex: Etapa 28" className="w-full bg-[#0a0d14] border border-[#1e2640] rounded-lg px-3 py-2 text-white" required />
              </div>
              <div>
              ...
              <div className="col-span-2 flex gap-3">
                <button type="submit" className="bg-[#e8ff47] text-black font-bold px-5 py-2 rounded-lg">Creeaza</button>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-500">Nu exista etape inca. Creeaza prima etapa!</div>
        ) : (
          <div className="space-y-3">
            {etape.map((etapa: any) => (
              <div key={etapa.id} className="bg-[#111520] border border-[#1e2640] rounded-xl px-6 py-4 flex items-center justify-between">
                <div>
...
// API etape
const etapeApi = `import { NextRequest, NextResponse } from "next/server"
...
// API sezoane
const sezoanApi = `import { NextRequest, NextResponse } from "next/server"
import { PrismaClient } from "@prisma/client"
export const dynamic = "force-dynamic"
const prisma = new PrismaClient()
...
fs.writeFileSync('app/admin/etape/page.tsx', etapePage)
fs.writeFileSync('app/api/admin/etape/route.ts', etapeApi)
fs.writeFileSync('app/api/admin/sezoane/route.ts', sezoanApi)
console.log('Gata!')