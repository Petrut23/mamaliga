...
export async function PATCH(req: NextRequest) {
  ...
  if (status === "OPEN") {
    const isWC = etapa.seasonId === WC_SEASON_ID
    await sendDiscordNotification(etapa, isWC)
    await sendEmailNotifications(etapa, isWC)
  }
...
async function sendDiscordNotification(round: any, isWC: boolean) {
  ...
  const message = {
    embeds: [{
      title: titlePrefix + " " + round.title + " este LIVE!",
      description: "E timpul să îți pui predicțiile!",
      color: 0xe8ff47,
      fields: [
        { name: "⏰ Deadline", value: deadline, inline: false },
        { name: "🔗 Link", value: "[Intră pe MamaLIGA](" + predictiiLink + ")", inline: false }
      ],
      footer: { text: "Mult succes! 🎯" }
    }]
  }
...
async function sendEmailNotifications(round: any, isWC: boolean) {
  ...
  try {
    ...
    for (const user of filteredUsers) {
      await transporter.sendMail({
        ...
        to: user.email,
        subject: `${titlePrefix} ${round.title} este LIVE!`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0d14; color: white; padding: 40px; border-radius: 12px;">
            ...
            <h2 style="color: white; font-size: 22px; margin-bottom: 16px;">${titlePrefix} ${round.title} este LIVE!</h2>
            ...
            <a href="${predictiiLink}" style="display: inline-block; background: #e8ff47; color: black; font-weight: bold; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-size: 16px;">
              Intră pe MamaLIGA →
            </a>
